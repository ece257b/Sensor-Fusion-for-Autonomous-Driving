...
def generate_lidar_bev_script(directories,
                              job_number):
    requirement_directory = directories['requirement_directory']
    lidar_file_dir_path = directories['lidar_file_dir_path']
...
cd {lidar_file_dir_path}

touch {log_file_path}

echo 'running for {town_folder_path}...'

python3 generate_lidar_bev.py --town_folder {town_folder_path} --lidar_folder_name "lidar" --save_folder_name "lidar_bev_img" > {log_file_path} 2>&1
...
lidar_file_dir_path = "/radar-imaging-dataset/P2SIF/data-collection/team_code/"
log_file_path = "/radar-imaging-dataset/P2SIF/data-collection/Scripts/logs/"
save_file_path = "/radar-imaging-dataset/P2SIF/data-collection/Scripts/LiDAR_Scripts/"
...
for scenario_path in scenario_paths:
    ...
    for town_path in town_paths:
        ...
        directories = {'requirement_directory': requirement_directory,
                        'lidar_file_dir_path': lidar_file_dir_path,
                        'town_folder_path': town_folder_path,
                        'log_file_path': log_file_path,
                        'save_file_path': save_file_path
                        }
        generate_lidar_bev_script(directories, job_number)
        print(f"job{job_number}.sh created")
        job_number += 1
print("All LiDAR BEV scripts generated")