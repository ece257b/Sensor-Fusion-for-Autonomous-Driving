...
export CHECKPOINT_ENDPOINT=${SAVE_DIR}/s3_2025-03-05/s3_Town03_Rep1/Results.json # output results file
export SAVE_PATH=${SAVE_DIR}/s3_2025-03-05/s3_Town03_Rep1 # path for saving episodes (comment to disable)
echo 'creating checkpoint and save_path directory...'
sleep 2s
mkdir -p ${SAVE_PATH}
touch ${SAVE_DIR}/s3_2025-03-05/s3_Town03_Rep1/Results.json
echo 'running leaderboard_evaluator_local.py now...'
sleep 3s

cp /radar-imaging-dataset/P2SIF/data-collection/Scripts/Job_Files//run_s3_Town03_repetition_1.sh ${SAVE_DIR}/s3_2025-03-05/s3_Town03_Rep1/
cd ${LEADERBOARD_ROOT}
