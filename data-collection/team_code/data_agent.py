...
import transfuser_utils as t_u # type: ignore
...
from ground_truth_data import get_ground_truth_data
...
class DataAgent(AutoPilot):
  ...
  def tick(self, input_data):
    ...
    # for half sweep with semantic lidar
    if self.last_semantic_lidar is not None:
      ego_transform = self._vehicle.get_transform()
      ego_location = ego_transform.location
      last_ego_location = self.last_ego_transform.location
      relative_translation = np.array([
      ...
      # Combine back and front half of LiDAR
      semantic_lidar_360 = np.concatenate((input_data['semantic_lidar'], semantic_lidar_last), axis=0)
    else:
      semantic_lidar_360 = input_data['semantic_lidar']  # The first frame only has 1 half
    ...
    ground_truth_data = get_ground_truth_data(actors)

    result.update({
        'lidar': lidar_360,
        'semantic_lidar': semantic_lidar_360,
        'rgb': rgb,
  ...
  def run_step(self, input_data, timestamp, sensors=None, plant=False):
    ...
    # Convert LiDAR and semantic lidar into the coordinate frame of the ego vehicle
    input_data['lidar'] = t_u.lidar_to_ego_coordinate(self.config, input_data['lidar'])
    input_data['semantic_lidar'] = t_u.semantic_lidar_to_ego_coordinate(self.config, input_data['semantic_lidar'])
    ...
    self.last_semantic_lidar = input_data['semantic_lidar']
...
#       self.augmented_vehicle_dummy.transform = transform_copy
  ...
  def save_sensors(self, tick_data):
    ...
    with open(self.save_path / 'ground_truth_data' / (f'{frame:04}.json'), 'w') as f:
      json.dump(tick_data['ground_truth_data'], f, indent = 4)

    # Save vehicle id in .txt file
    with open(self.save_path / 'ego_vehicle_id.txt', 'w') as f:
      f.write(str(self._vehicle.id))
    ...
    # Specialized Semantic Lidar compression format
    ...
    header.offsets = np.min(tick_data['semantic_lidar'][:, :3], axis=0)
    header.scales = np.array([self.config.point_precision, self.config.point_precision, self.config.point_precision])

    with laspy.open(self.save_path / 'semantic_lidar' / (f'{frame:04}.laz'), mode='w', header=header) as writer:
      point_record = laspy.ScaleAwarePointRecord.zeros(tick_data['semantic_lidar'].shape[0], header=header)
      point_record.x = tick_data['semantic_lidar'][:, 0]
      point_record.y = tick_data['semantic_lidar'][:, 1]
      point_record.z = tick_data['semantic_lidar'][:, 2]
      point_record.cosine = tick_data['semantic_lidar'][:, 3]
      point_record.index = tick_data['semantic_lidar'][:, 4]
      point_record.sem_tag = tick_data['semantic_lidar'][:, 5]
      # pdb.set_trace()
      writer.write_points(point_record)
    # np.save(f'{self.save_path}/semantic_lidar/{frame:04}.npy', tick_data['semantic_lidar'])
    
    # fig, ax = plt.subplots(1, 2)
    # # ax = fig.add_subplot(111, projection='3d')
    # # ax.scatter(x, y, z, s=0.1, c='r', marker='o')  # You can customize the color and marker type
    # x = tick_data['semantic_lidar'][:, 0]
    # y = tick_data['semantic_lidar'][:, 1]
    # sem = tick_data['semantic_lidar'][:, 5]
    
    # # ax[0].scatter(tick_data['semantic_lidar'][:, 0], tick_data['semantic_lidar'][:, 1], s=0.1, c=tick_data['semantic_lidar'][:, 5])
    # ax[0].scatter(x, y, s=0.1)
    # ax[0].scatter(x[sem==10], y[sem==10], s=0.1, c='y')
    # ax[1].scatter(tick_data['lidar'][:, 0], tick_data['lidar'][:, 1], s=0.1)
    # plt.show()
    # pdb.set_trace()
