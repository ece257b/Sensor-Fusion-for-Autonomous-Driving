...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for data in train_loader:
            image_tensor = data["radar"]
            captions = data["caption"]
            curr_loss = 0
            
            with torch.no_grad():
                radar_processed = preprocess(image_tensor).to(device)
                caption = clip.tokenize(captions).to(device)
                caption_features = model.module.encode_text(caption)

            image_features = model.module.encode_image(radar_processed)
        ...
        if epoch % 2 == 0:
            torch.save(model, f"./models/{save_name}_train_radar_epoch{epoch}.pth")
        
    # Save the model
    if rank == 0:
        torch.save(model, f"./models/{save_name}_train_radar.pth")
        np.save(f"./models/{save_name}_train_radar_loss.npy", np.array(loss_list))
